...
export default function SettingPageModule ({userDoc}: PropsType) {
  ...
  const getItemCount = async () => {
    ...
    try {
      ...
      const transactionCount = await getDocumentCount(`user-transactions/${userDoc.userId}/transactions`);
...
const BadgeItem = ({label, icon, item, className, loadingGetItem}: BadgePropsType) => {
  return (
    <div
      className={cn(
    ...
    >
      ...
      <div className="flex flex-col gap-2 text-center items-center">
        <Label>{label}</Label>
        {loadingGetItem ? <Skeleton className="w-14 h-3 bg-primary-hover rounded-md" /> : <Label>{item}</Label>}
      </div>
    </div>
  );
}
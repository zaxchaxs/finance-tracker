...
export default function Home() {
  ...
  return (
    <main className="min-h-screen p-5 text-secondary bg-primary w-full flex flex-col items-center justify-between py-10 pb-14"> 
      <h1 className="text-3xl p-4 text-primary text-center w-full font-lilitaOne">Zaxch Finance Tracker</h1>
      <div>
      ...
      <div onClick={logout} className={`gap-4 flex p-4 ${currUser ? "" : "hidden"}`}>
        <button className="p-2 px-4 rounded-xl font-lilitaOne bg-danger hover:bg-danger-hover text-lightGreen">Logout</button>
      </div>
    </main>
  );
}
